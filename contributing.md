...
4. `docker-compose run rails bundle exec rspec spec` を実施し、すべてのテストが通るか確認する。
5. `docker-compose run rails bundle exec rubocop` 及び `docker-compose run webpacker yarn lint` を実施し、コーディング規約に従っているかチェックする。
